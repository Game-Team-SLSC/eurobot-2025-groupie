...
namespace maqueen {
    ...
    function patorlState(): number {
        switch (i) {
            case 1:
                x = pins.digitalReadPin(DigitalPin.P13) == 0 ? 0x10 : 0;
                break;
            case 2:
                x = pins.digitalReadPin(DigitalPin.P13) == 1 ? 0x11 : 0;
                break;
            case 3:
                x = pins.digitalReadPin(DigitalPin.P14) == 0 ? 0x20 : 0;
                break;
            default:
                x = pins.digitalReadPin(DigitalPin.P14) == 1 ? 0x21 : 0;
                break;
        }
        i += 1;
        if (i == 5) i = 1;
...
namespace Maqueen_V5 {
    ...
    export function patrolling(patrol: Patrolling) {
        let allBuffer = pins.createBuffer(2);
        if (patrol == Patrolling.ON) allBuffer[1] = 1;
        else allBuffer[1] = 0;
    ...
    */
    ...
    export function getBatteryData(
        type: BatteryType = BatteryType.Alkaline
    ): number {
        let allBuffer = pins.createBuffer(2);
